...
def index():
    ...
    books_list = db.execute("SELECT title, author FROM list WHERE user_id = ? AND read=0", session["user_id"])
...
def recommend():
    ...
    db.execute("CREATE TABLE IF NOT EXISTS hardcover_fiction (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, title TEXT NOT NULL, author TEXT NOT NULL, book_image TEXT NOT NULL)")

    # db.execute("INSERT INTO table (id, name, age) VALUES(1, "A", 19) ON DUPLICATE KEY UPDATE name="A", age=19")
    recommend_list = {}

    # 'title': None, 'author': None, 'description': None, 'rank': None, 'book_image': None
    for entry in recommends["results"]["books"]:
        db.execute("INSERT INTO hardcover_fiction (title, author, book_image) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE title=?, author=?, book_image=?",
        entry['title'], entry['author'], entry['description'],
        entry['title'], entry['author'], entry['description'])
